...
// ✅ GLOBAL API — token sent automatically, no axios/withCredentials needed
import api from '../../api';
...
export const IndividualDashboard = () => {
  ...
  // ✅ FIXED: /api/dashboard (removed /individual/)
  const fetch_ = async () => {
    try {
      console.log("📊 Fetching individual dashboard...");
  ...
  // ✅ FIXED: export using api, no withCredentials
  const handleExport = async (fmt) => {
    try {
      ...
      l.href = u;
      l.download = fmt === 'csv' ? 'transactions.csv' : 'report.pdf';
      l.click();
      console.log(`✅ Export ${fmt} done`);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[var(--p-bg)] pb-20">
      ...
      {/* ── Header ── */}
      <header className="sticky top-0 z-40 bg-[var(--p-surface)]/90 backdrop-blur-xl border-b border-[var(--p-border)]">
        <div className="flex items-center justify-between h-14 max-w-4xl mx-auto px-4">

          <span className="font-bold text-[var(--p-text)] text-sm">Capital Care AI</span>
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              className="p-2 rounded-lg text-[var(--p-text-muted)] hover:bg-[var(--p-border-subtle)] hover:text-[#1D9E75]"
            ...
            <div className="relative group">
              <button
                className="p-2 rounded-lg text-[var(--p-text-muted)] hover:bg-[var(--p-border-subtle)]"
                data-testid="export-button"
              >
                <DownloadSimple size={20} />
              </button>
              <div className="absolute right-0 top-10 bg-[var(--p-surface)] border border-[var(--p-border)] rounded-xl shadow-lg py-1 w-36 hidden group-hover:block z-50">
                <button
                  onClick={() => handleExport('csv')}
                  className="w-full text-left px-3 py-2 text-xs text-[var(--p-text-secondary)] hover:bg-[var(--p-border-subtle)]"
            ...
            {/* Switch persona */}
            <button
              onClick={async () => { await setPersona(null); nav('/persona?switch=1'); }}
              className="p-2 rounded-lg text-[var(--p-text-muted)] hover:bg-[var(--p-border-subtle)]"
      ...
      <main className="max-w-4xl mx-auto px-4 py-5 space-y-5">
        ...
        {loading ? (
          <div className="flex justify-center py-20">
            <div className="h-10 w-10 animate-spin rounded-full border-[3px] border-[#1D9E75] border-r-transparent" />
          </div>
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-3">
              {kpis.map((k, i) => (
                <div
                  key={k.label}
                  className={`animate-fade-up stagger-${i + 1} bg-[var(--p-surface)] border border-[var(--p-border)] p-4 transition-all hover:shadow-md`}
            ...
            {d.category_breakdown?.length > 0 && (
              <div
                className="bg-[var(--p-surface)] border border-[var(--p-border)] p-4"
                style={{ borderRadius: 'var(--p-radius)' }}
                    ...
                    {d.category_breakdown.slice(0, 5).map((c, i) => (
                      <div key={c.name} className="flex items-center justify-between text-xs">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-[var(--p-text-secondary)]">{c.name}</span>
                        </div>
              ...
              >
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <LineChart data={d.monthly_series}>
                    ...
                    <Tooltip
                      contentStyle={{ background: theme === 'dark' ? '#1E293B' : '#FFF', border: `1px solid ${theme === 'dark' ? '#334155' : '#E2E8F0'}`, borderRadius: '8px', fontSize: '11px' }}
                  ...
                  return (
                    <div key={g.id} className="mb-3 last:mb-0">
                      <div className="flex justify-between text-xs mb-1">
                        <span className="text-[var(--p-text)]">{g.name}</span>
                        <span className="text-[var(--p-text-muted)]">{formatINR(g.saved)} of {formatINR(g.target)}</span>
                      </div>
                      <div className="w-full h-2 bg-[var(--p-border-subtle)] rounded-full overflow-hidden">
      ...
      {/* Bottom Nav */}
      <nav
        className="fixed bottom-0 left-0 right-0 z-50 bg-[var(--nav-bg)] backdrop-blur-xl border-t border-[var(--p-border)]"
        data-testid="bottom-nav"
      >
        <div className="flex items-center justify-around h-14 max-w-4xl mx-auto">
          {[
            { icon: House,       label: 'Home',         path: '/individual' },
            { icon: Receipt,     label: 'Transactions', path: '/individual/transactions' },
            { icon: Target,      label: 'Goals',        path: '/individual/goals' },
            { icon: ChartLine,   label: 'Reports',      path: '/individual' },
            { icon: UserCircle,  label: 'Profile',      path: '/persona' },
          ].map(i => (
            <button
              key={i.label}
              onClick={() => nav(i.path)}
              className={`flex flex-col items-center gap-0.5 w-16 py-1
                ${window.location.pathname === i.path ? 'text-[#1D9E75]' : 'text-[var(--p-text-muted)]'}`}
              data-testid={`nav-${i.label.toLowerCase()}`}
            >
              <i.icon size={20} weight={window.location.pathname === i.path ? 'fill' : 'regular'} />
